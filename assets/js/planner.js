/* ---------------------------------------------------------------------------
   Skill tree build planner.

   Runs on data/skilltree.js, which tools/make_skilltree_data.py ports directly
   from the mod's SkillTreeRegistry — so allocation rules, costs and stat values
   match the game rather than approximating it.

   Builds are shared as a bitmask over the node array, base64url in the URL
   hash, which keeps a full 117-node build inside ~20 characters.
--------------------------------------------------------------------------- */
(function () {
  "use strict";
