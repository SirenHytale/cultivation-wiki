...
默认情况下，光柱之色与殿下灵脉相符。宗主或长老可改为**升起一面旗帜** —— 或在宗门菜单的**山门旗帜**选择栏中选取，或以 `/sect banner set <名称>` 指定。模组随附六面旗帜：
...
- `/sect banner` 不带参数时，会列出本宗可升起的旗帜，并标出当前所悬的那一面。简称（`azure`）与完整 id 皆可用；`/sect banner set none` 则归回**本色灵光**。
...
自 **0.7.2** 起，宗门可以用 `/sect dao set {element}` 拉十大[道](/cultivation/dao/)之一为本宗之道。在宗门所辖的地界上修行 —— 山门，以及任何开着开关的[产业](#buildings) —— 灵气便偏向那一元素。

| 设置 | 默认 | 作用 |
| --- | --- | --- |
| `Sect-Dao-Matched-Multiplier` | 1.3 | 修士本就修此道时的采气倍率。 |
| `Sect-Dao-Ambient-Multiplier` | 1.1 | 修士不修此道时的采气倍率。 |
| `Sect-Dao-Affinity-Interval-Seconds` | 8 | 多久拉一次亲和。 |
| `Sect-Dao-Affinity-Per-Interval` | 2.5 | 一次拉多少，汇入常规的[偏移规则](/cultivation/dao/)。 |

这比[地貌](/cultivation/dao/#terrain)**刻意更强** —— 每 8 秒 2.5 点，对比地貌的每 10 秒 1.5 点。宗门的地是刻意经营的，在其中坐上一季的弟子，本就应当带着宗门的元素走出来。宗门如今有了性情，而它的地会把那性情教给人。

执行 `/sect dao` 不带参数可查看当前之道，`set none` 则清除。未拉本道的宗门，地界与 0.7.2 之前一样无性。

#### 共修之进境 {#shared-progression}

宗门之所以上进，是因为**门下弟子在上进**。每有一名成员晋一阶或破一境，宗门便积下一分：

- 成员晋阶：**10 点**（`Sect-Xp-Per-Member-Advancement`）。
- 成员破境：**60 点**（`Sect-Xp-Per-Member-Breakthrough`）。
- 第 *N* 级需 **200 × 1.6^(N−1)** —— 即第 2 级 200，第 3 级 320，第 4 级 512，依此类推。

共修之进境就这么多，而它的形状正是重点：无物可捐，也无处可刷。**招人、且真把人养出来，才是建宗之道** —— 一山门永不晋阶的炼体期弟子，一分也挤不出来。

等级决定宗门可以营建哪些[产业](#buildings)。至于可占**多少地**，那是另一回事 —— 自 **0.7.3** 起，答案取决于[门下有多少弟子](#territory)。

#### 宗门领地 {#territory}

自 **0.7.3** 起，宗门所据的是**连成一片的地界**，而非一座大殿加几处零散标记。它自大殿向外逐块延展，所占之地皆受[领地守护](/cultivation/land/)。

| 配置 | 默认 | 作用 |
| --- | --- | --- |
| `Sect-Chunks-Base` | 3 | 宗门起始所占区块数，**含大殿在内** |
| `Sect-Chunks-Per-Member` | 1 | **开宗者之外**每名弟子额外增加的区块数 |
| `Sect-Chunks-Max` | 32 | 上限，无论宗门壮大到何等规模 |

故一人开创的宗门，占有大殿与其周围两块地。收下第二名弟子，便可再占第三块。**拓土靠人**，这与推动宗门等级的原是同一件事 —— 也意味着一个荒废的独人宗门无法长踞广地。

##### 于舆图上占地

宗门菜单中载有**领地舆图**：以大殿为中心的一片区块。朱红为宗门已占之地，淡金为下一步可取之地，其余皆不可点，且以提示说明缘由 —— 或为他宗所占，或未与已有之地相邻，或宗门已无余地。

点击区块即可选中，舆图下方一行随即列出可行之事：空地为**占取区块**，已占之地则为**指定用途**与**舍弃**。

**宗门之地向外延展。** 唯有与已有之地**正相邻**（不含斜角）的区块方可占取，故宗门之地始终是一片可以走通的连片土地，而非散落于大陆各处的标记。大殿是根本，须先占下。

##### 空地，与有用途之地

占地与营建是**两步**。新占的区块是**空地** —— 它计入上限、受守护、其大道开关亦可用，只是尚无用途。日后可用 `/sect building assign {type}` 或舆图上的「指定用途」定其所用，而 `assign none` 则撤回用途而不弃其地。

指定用途**绝不会**重新套用某类建筑的默认规矩。你所设的[领地规矩](/cultivation/land/)与大道开关都是你的；改个用途，不会悄悄将它解封。

##### 跨越疆界

走上宗门之地时，屏上会显出**通告**，标明该宗之名；走出时亦然。本门疆界以宗训相迎；他宗疆界则只告知你正立于谁家的地上。它只在跨越之时显现，你在其中站立或走动皆不会再现。

#### 宗门产业 {#buildings}

宗门所占的区块可以指定一个**用途**。每一种都有自己的打坐倍率，以及一个需要等候的宗门等级：
...
站在目标地块上用 `/sect building claim` 占为空地，或用 `/sect building claim {type}` 一并置下产业。`/sect building assign {type}` 为已占之地指定用途，`/sect building abandon` 舍弃，`/sect building` 不带参数则列出已有的。

每一处产业都带着**一道重于其余的开关**：其地是否向内中修士压本宗之道。用 `/sect building dao` 切换。

> **中立之地才是重点。** 关掉开关的产业即为中立之地：打坐倍率照旧生效，却不再拉扯修士的元素。这正是宗门如何**接纳一位所修不同的好友**而不把他慢慢换成另一个人。客卿院、丹药园与炼器坊默认关闭，正是为此 —— 一座会您您同化客人的客卿院，算什么客卿院。

反过来也同样是刻意的。想让全宗归于一元素的，就多置藏经阁与祖师祠、开尽开关；想广纳四方的，就置客卿院。两种都玩得转，而产业就是宗门表明自己是哪一种的方式。
...
| `/sect banner [set <banner>]` | 不带参数列出本宗可升之旗；`set` 升起一面，`set none` 收起（宗主或长老）。 | `cultivation.sect` |
| `/sect war [status 或 declare <宗门>]` | 不带参数或 `status` 查看本宗当前战况；`declare` 发起围攻（宗主）。见[宗门攻伐](/cultivation/wars/)。 | `cultivation.sect` |
