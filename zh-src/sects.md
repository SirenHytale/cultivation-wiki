...
自 **0.7.2** 起，宗门可以用 `/sect dao {element}` 拉十大[道](/cultivation/dao/)之一为本宗之道。在宗门所辖的地界上修行 —— 山门，以及任何开着开关的[产业](#buildings) —— 灵气便偏向那一元素。
...
执行 `/sect dao` 不带参数可查看当前之道，传入 `none` 则清除。未拉本道的宗门，地界与 0.7.2 之前一样无性。
...
等级换来的是[产业](#buildings)：每级一个位，最多六处。

#### 宗门产业 {#buildings}

山门之外，宗门可在自己的地块上置下产业。每一种都有自己的打坐倍率，以及一个需要等候的宗门等级：
...
站在目标地块上用 `/sect building claim {type}` 置下，`/sect building abandon` 舍弃，`/sect building` 不带参数则列出已有与尚可置下的。
