...
**0.7.4 新增。** 凡要改变一名修士**是什么**、而非他**做什么**的，皆挂在此处 —— 血脉、体质、特殊体质。一次注册即回答一组具名的通道，Cultivation 则在自己各处关口逐一相询：
...
每个方法都有一个「不予置评」的默认实现，故你只需实现自己在意的那几条。通道共十六条，涵盖灵气吸纳与打坐、所施与所受的伤害、空手伤害、伤害类型免疫、[蓄力](/cultivation/techniques/#gathered-arts)时长、大道元素之锁、大道亲和、阴阳偏移、阴阳容差、仪轨难度、于岩浆中打坐，以及[灵光](/cultivation/auras/)大小 —— 另有两条并不改动数值，而是向总览页添入条目。

**为何是此处而非事件。** 事件适于回应一个**瞬间**；此处所对的是关于一名玩家的一桩**常驻事实**，它会一举重定十几套彼此无关的系统。若以事件为之，便要在每一套上各挂一个监听器，且各自反复推导同一桩事实。

各注册来源之间，倍率**相乘**，布尔值**取或**，故两个扩展各加 10% 得到的是 ×1.21 而非 ×1.20 —— 这与 Cultivation 自身内部倍率的叠加方式一致。某个来源若抛出异常，会被捕获并记录一次，此后不再理会，故一个有缺陷的扩展无法拖垮整条伤害管线。

你的方法在世界线程上被调用，有时更在伤害管线内部，故请只做一次组件读取与一些算术。当无人注册时，每条通道都会立即返回其中性值。

[Cultivation: Sacred Bodies](/cultivation/sacred-bodies/) 全然建于此注册表之上，别无其他，值得当作可仿的范式来读：它以**一次组件读取**回答全部通道，并向总览页添入自己的具名条目，使体质显现为可读的文字，而非玩家数值上一处无从解释的变动。
