...
- You cannot claim ground warded against you by someone else's Mountain-Guarding [Formation](/formations/).
- Any Spirit Vein tier will do, but the vein under your claim decides how fast the spring fills.

`/cultivation abode abandon` gives the claim up outright - and the banked spring is lost with it. Run on its own it does not abandon anything: it tells you how much Qi the spring is currently holding, so you can go and `collect` it first. Only `/cultivation abode abandon confirm` actually walks away.
...
Outsiders also cannot inscribe [Formations](/formations/) on a sustained abode's ground. A lapsed abode is just an empty cave until its owner feeds it again - no multipliers, no ward.

<Tag>v0.7.0</Tag> Your claimed abode is **marked on Hytale's own world map**, so finding your way home no longer means remembering coordinates. The pin is yours alone - other players' maps do not show it.
...
- **35%** chance for **surviving** a tribulation strike (`SpiritStone-Tribulation-Drop-Chance`) - the heavens' lightning crystallizes the Qi it scatters. See [Tribulations](/tribulations/).
...
With `Sect-Hall-Spring-Enabled` on (the default), a [Sect](/sects/)'s claimed hall grows a **shared** Spirit Spring of its own, on top of every member's personal abode:
...
- It follows the hall: moving the hall moves the spring, and **losing the hall in a siege hands the spring, and everything banked in it, to the victor**. See [Sect Wars](/wars/).
...
<Panel title="Commands">
  ...
  | `/cultivation abode abandon [confirm]` | Reports what the spring holds; `confirm` gives up the claim and loses it. | `cultivation` |
</Panel>
...
Every value on this page lives in the society config - see [Society Config](/config-society/) for the full field reference, and [Commands](/commands/) for every command in the mod. Abodes interlock with [Sects](/sects/), are warded against by [Formations](/formations/), and change hands during [Sect Wars](/wars/).
