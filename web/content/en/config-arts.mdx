...
The eight files in `mods/Siren_Cultivation/Arts/` cover everything a cultivator practices, crafts, tempers, and binds to themselves - the [Dao](/dao/) they walk, the [Techniques](/techniques/) they perform, the [Fist Arts](/techniques/#fist-arts) they train bare-handed, the [Manuals](/manuals/) that teach them, the pills they brew in [Alchemy](/alchemy/), the [Weapon Refinement](/refinement/) they endure, their [Life-Bound Treasures](/lifebound/), and their [Spirit Beasts](/beasts/).
...
The elemental daos, the Yin-Yang balance, weather resonance, the Devil/Righteous path split, and Karma (业力), found in the path `mods/Siren_Cultivation/Arts/DaoConfig.json`. See [The Dao](/dao/) and [Karma](/karma/) for what these do in play.
...
The land's pull on your element. See [The Ground You Sit On](/dao/#terrain).

| Variable Name | Default Value | Description |
| --- | --- | --- |
| `Terrain-Dao-Enabled` | true | False leaves the land inert. |
| `Terrain-Matched-Multiplier` | 1.25 | Qi gained where the land matches the cultivator's own dao. |
| `Terrain-Ambient-Multiplier` | 1.08 | Qi gained on merely elemental ground. |
| `Terrain-Affinity-Interval-Seconds` | 10 | How often meditating adds affinity toward the land's element. |
| `Terrain-Affinity-Per-Interval` | 1.5 | How much it adds. Gentler than a kill on purpose. |
| `Terrain-Element-Keywords` | 60 pairs | `keyword:ELEMENT`, matched as case-insensitive substrings of the biome's own name, first hit wins. Works with a generator mod's biomes. |

## Technique Config

The rule set behind every active cultivation art, found in the path `mods/Siren_Cultivation/Arts/TechniqueConfig.json`. See [Techniques](/techniques/) for what each one does.
...
| "Techniques" | An array of the 28 built-in rules (see below). | One entry per technique. Each entry's `Id` must match a technique id for the manager to apply it as an override. |
| "Technique-Hotkeys-Enabled" | true | Whether cultivators may fire techniques from [a keybind](/keybinds/) (a modifier plus a key) rather than only from `/cultivation technique`. Independent of "Techniques-Enabled". |
| "Technique-Hotkeys-Q-E-R-Enabled" | false | Whether Q, E and R may be bound at all. Off by default because taking one over costs a player dropping an item, the inventory, or their hotbar block set — the number row only costs a hotbar switch, which the mod immediately undoes. |
| "Description-Technique-Hotkeys" | A long string explaining the values above. | Which keys and modifiers are bindable and why the set is what it is, that there is no 0 key, the default bindings, and that the modifier arms on the press for half a second rather than while held. |
...
| "Requires-Unlock" | true | When true this technique cannot be performed until the player has [come by](/techniques/#unlocking) it — a [manual](/manuals/), enlightenment, a breakthrough, or a sect inscription. **Set on all 28 built-ins since 0.7.4**; set it false to make an art freely available at its realm. This key replaced `Requires-Manual`, and the rename is deliberately the migration: an old config's stale `"Requires-Manual": false` is read as an unknown key and ignored, so the new default applies rather than being silently overridden. |
| "Charged" | false | When true the art is pressed once to begin gathering and again to loose it, growing stronger the longer it is held. Its numbers ride along as the `Charge*` params. Set on the three [gathered arts](/techniques/#gathered-arts). |
| "Requires-Unarmed" | false | When true the art refuses while a weapon is in hand. Set on the three [fist arts](/techniques/#fist-arts). What counts as a free hand is governed by `Unarmed-Requires-Empty-Hand` in the fist config. |
...
These are the 28 built-in rules written to the file on first run.
...
The five-rung ladder every art climbs. See [Mastery](/techniques/#mastery) for what it means in play.

| Variable Name | Default Value | Description |
| --- | --- | --- |
| `Mastery-Enabled` | true | Whole-system switch. False returns every art to realm-only strength, exactly as 0.7.1 behaved. |
| `Mastery-Xp-Per-Use` | 2 | Practice XP banked by one *successful* use. A refused activation teaches nothing. |
| `Mastery-Xp-Per-Qi-Cost` | 0.05 | Extra XP per point of the art's Qi cost, so an expensive art climbs faster per cast. |
| `Mastery-Stages` | 5 entries | The ladder itself. Each rung carries `Name`, `RequiredRealm`, `RequiredXp`, `RequiredManuals`, `PowerMultiplier`, `QiCostMultiplier` and `CooldownMultiplier`. |

<Note kind="warn" title="The ladder is capped at five">
  A config with more than five rungs is clamped to the first five, and one with none falls back to a single flat rung. The cap exists because the UI and the lang keys only cover five — an add-on calling `registerMasteryStage` past that gets `false` back rather than a silently ignored rung.
</Note>
...
The bare-handed damage ladder and the arts that ride on it (拳法), found in the path `mods/Siren_Cultivation/Arts/FistConfig.json`. New in **0.7.4**. See [Fist Arts](/techniques/#fist-arts).

| Variable Name | Default Value | Description |
| --- | --- | --- |
| "Fist-Arts-Enabled" | true | Switches the whole ladder off, including its damage bonus. The three fist arts still exist as techniques and can be disabled individually in the technique config. |
| "Unarmed-Requires-Empty-Hand" | false | What counts as a free hand. False means anything that is not a weapon, so a torch or a stack of stone still lets a fist art fly; true demands a strictly empty hand. |
| "Max-Level" | 100 | The ceiling on the ladder. The full damage bonus is paid out only there. |
| "Base-Xp-Requirement" | 30.0 | XP needed for level 2. The requirement for level N is this times `Xp-Growth-Rate` to the power of N-1. |
| "Xp-Growth-Rate" | 1.075 | How steeply the requirement compounds. Small changes here move the ceiling a very long way. |
| "Xp-Per-Damage-Dealt" | 0.5 | XP per point of damage that **actually got through**. Hitting something armored teaches less than hitting something soft, and a punch a shield ate whole teaches nothing. |
| "Damage-Bonus-Percent-At-Max" | 220.0 | Extra unarmed damage at `Max-Level`, as a percentage. It applies to your fists and to the three fist arts, and never to a weapon. |
| "Damage-Bonus-Percent-Per-Realm" | 12.0 | Further unarmed damage for every [realm](/realms/) you have broken into, on top of the ladder. |

## Manual Config

Lootable books that permanently teach a technique or a skill-tree node, found in the path `mods/Siren_Cultivation/Arts/ManualConfig.json`. See [Manuals](/manuals/).
...
| "TechniqueId" | "" | The technique reading it teaches, if any. Since 0.7.4 every built-in art sets `Requires-Unlock`, so a technique manual is a real unlock rather than a curiosity. |
...
The four pills and what they do, found in the path `mods/Siren_Cultivation/Arts/AlchemyConfig.json`. See [Alchemy](/alchemy/).
...
Tempering a weapon in tribulation lightning (炼器), found in the path `mods/Siren_Cultivation/Arts/RefinementConfig.json`. See [Weapon Refinement](/refinement/).
...
The XP curve and combat bonuses for Life-Bound Treasures, found in the path `mods/Siren_Cultivation/Arts/LifeBoundConfig.json`. See [Life-Bound Treasures](/lifebound/).
...
The natures a treasure can roll. See [Traits](/lifebound/#natures).

| Variable Name | Default Value | Description |
| --- | --- | --- |
| `Life-Bound-Traits-Enabled` | true | False returns every treasure to the flat damage/reduction split of 0.7.1. |
| `Life-Bound-Traits` | 6 entries | Each carries `Id`, `Slot` (`WEAPON`, `ARMOR` or `ANY`), `BaseAmount`, `AmountPerLevel`, `MaxAmount`, `Weight`, and optionally `UnlockTechnique` plus `UnlockTechniqueLevel`. |

A nature is rolled **once, at binding**, weighted by `Weight` and restricted by `Slot`, and never re-rolled. **Weight 0 retires a trait** from the roll without breaking treasures already carrying it — the safe way to remove one from a live server.

## Beast Config

Spirit Beast companions (灵兽), found in the path `mods/Siren_Cultivation/Arts/BeastConfig.json`. See [Spirit Beasts](/beasts/).
...
### Growth: arts, evolution and mounts

The 0.7.2 half of the beast system. See [Beast Arts](/beasts/#arts), [Evolution](/beasts/#evolution) and [Mounts](/beasts/#mounts).

| Variable Name | Default Value | Description |
| --- | --- | --- |
| `Beast-Arts-Enabled` | true | Whole-system switch for companion arts. |
| `Beast-Arts-Owner-Commanded` | true | Whether `/cultivation beast art` works at all. |
| `Beast-Arts-Autonomous` | true | Whether a beast uses its arts unprompted in a fight. |
| `Beast-Arts-Autonomous-Range` | 12 | How close a fight must be for it to act on its own. |
| `Beast-Evolution-Enabled` | true | Whole-system switch for evolution. |
| `Beast-Mounts-Enabled` | true | Whole-system switch for rideable companions. |

Which creature learns what is not here — it is on the species itself, in `BeastSpeciesList`: `Arts` lists the art ids it can grow into, `EvolvesTo` plus `EvolveRealm` name its next form, and `MountRealm` makes it rideable. That is what lets an add-on register an art and still leave the owner to decide which beasts get it.
