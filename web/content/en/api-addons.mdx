...
[Events](/api-events/) let you reshape what Cultivation does. [Registries](/api-registries/) let you add content it treats as its own. The three hooks on this page go further: they let another mod **take over** a whole layer of Cultivation and keep everything else running on top.
...
**Everything else in the mod keeps working.** [Sects](/sects/), [the Dao](/dao/), [techniques](/techniques/), [spirit beasts](/beasts/), [formations](/formations/), [cave abodes](/dwelling/), [duels](/duels/), [alchemy](/alchemy/), [refinement](/refinement/) and the [skill tree](/skilltree/) all sit on top of whatever ladder is installed.
...
| Flavour | Ritual |
| --- | --- |
| **Advancement** | The routine step. Shorter, lower Spirit Vein requirement, no [tribulation](/tribulations/) lightning by default. |
...
### Profiles under a provider

<Tag>v0.7.0</Tag> The interface gained one `default` method:
...
And that default **refuses [profile](/profiles/) switching outright** while your provider is installed - the player is told another mod governs progression here. That is deliberate: a switch swaps the player's `CultivationComponent`, race, dao, skill tree and techniques underneath you, and a provider that has not planned for it would be left describing the wrong cultivator - half-performed is worse than refused.

To lift the refusal, do both of these and then return `true`:

- Handle `ProfileEvents.PreProfileSwitchEvent` - save your own state for the profile being left.
- Handle `ProfileEvents.ProfileSwitchEvent` - load your state for the arriving one.

Profile listeners are the one place in the API allowed to **write components directly through the `Store`** - they run on the world thread outside any ticking system, precisely so a provider can do this bookkeeping. See [Events](/api-events/) for the full `ProfileEvents` table. The one exception to the refusal is the operator's [test sandbox](/profiles/), which is allowed even under a provider - it cannot change your addon's view of the player, and the command says so.

<Divider />

## CultivationTheme \{#cultivationtheme\}
...
The [rankings](/realms/) and `/cultivation top` sort on that flattened level, so a replacement progression ranks correctly against itself. For the built-in ladder the ordering is exactly what realm-then-stage always produced.
