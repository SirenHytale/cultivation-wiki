...
**Refinement (炼器)** is the art of tempering a weapon in heavenly tribulation lightning until it takes on a [Dao](/dao/) element. Run `/cultivation refine {element}` (alias `temper`) with the weapon in hand and you sit down into a ritual much like a breakthrough - the lightning falls on **you**, not the blade.
...
Every value on this page lives in the refinement config - see [Config](/config-arts/) - and the commands are on the [Commands](/commands/) page.
