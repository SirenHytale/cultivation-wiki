...
**Spirit Sense (灵气感知)** is a cultivator's perception of the Qi in the land around them, and above all **whether the ground they are standing on can carry their next rank-up**. Still your mind and the ambient Qi takes shape — motes drifting up from the ground you sit on, and a beacon standing over a [rich or dragon vein](/qi-gathering/) from across the landscape. New in v0.6.0, and switched on by default (`Spirit-Sense-Enabled`).
...
The whole feature is one switch: `Spirit-Sense-Enabled` in `Cultivation/SpiritVeinConfig.json`. Turning it off leaves [Qi gathering](/qi-gathering/) entirely unchanged — the sense only ever *reads*.
...
Full settings on the [Cultivation config page](/config-cultivation/).
