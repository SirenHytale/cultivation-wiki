...
Cultivation exposes 160 events across thirteen `*Events` classes in `plugin.siren.API`. Nearly every mechanic in the mod is exposed twice - once as a cancellable `Pre*` event fired before the change, and once as a plain post-event fired after it is committed.

Every class follows the conventions documented on `CultivationEvents`, so learning one teaches you all eleven.
...
`plugin.siren.API.CultivationEvents` - 28 events covering Qi, meditation, rituals, breakthroughs, advancements, demotions, tribulations, the Heart-Devil Trial, Qi Deviation, races, Life-Bound Treasures and the skill tree. Player-facing docs: [Cultivation Realms](/realms/), [Qi Gathering](/qi-gathering/), [Tribulations](/tribulations/), [Races](/races/), [Life-Bound Treasures](/lifebound/), [Skill Tree](/skilltree/).
...
`plugin.siren.API.DaoEvents` - 14 events covering elements, drift, the Yin-Yang balance, moral paths and karma. Player-facing docs: [The Dao](/dao/) and [Karma](/karma/).
...
`plugin.siren.API.TechniqueEvents` - 13 events covering performing and learning arts (功法), the Sword Flying toggle, and the timed combat buffs. Player-facing docs: [Techniques](/techniques/) and [Manuals](/manuals/).

These fire for third-party techniques registered through [`CultivationAPI.registerTechnique`](/api-registries/) exactly as they do for the built-in ones - the gate, cost and cooldown pipeline is shared. Two enums live here: `BuffType` (`IRON_BODY`, `QI_INFUSION`, `QI_BARRIER`, `CLOUD_STEP`) and `FlightStopReason` (`TOGGLE`, `QI_EXHAUSTED`, `DEATH`).
...
| `PreTechniqueMasteryAdvanceEvent` | Yes | An art is about to rise a mastery rung. Cancel to hold it where it is — the XP and the studied manuals stay banked, so it simply tries again the next time anything about it changes. | `ref()`, `player()`, `techniqueId()`, `stage()` (the rung it is rising *to*). |
| `TechniqueMasteryAdvanceEvent` | No | An art’s mastery rose a rung. | `ref()`, `player()`, `techniqueId()`, `stage()`. |

Config keys these override: `PreTechniquePerformEvent` mirrors the per-technique `Qi-Cost` and `Cooldown-Seconds` entries in the technique rules (see [Config](/config/)), and the `/cultivation technique <id>` command on the [Commands](/commands/) page. `PreTechniqueLearnEvent` fires for every art a player comes by, whichever of the four unlock routes produced it — reading a [manual](/manuals/), enlightenment while meditating, a breakthrough, or a sect inscription — so a listener does not have to care which.

<Divider />

## Items

`plugin.siren.API.ItemEvents` - 12 events covering cultivation drops, pills, cores, manuals and weapon refinement (炼器). Player-facing docs: [Qi Gathering](/qi-gathering/), [Alchemy](/alchemy/), [Manuals](/manuals/), [Weapon Refinement](/refinement/).
...
`plugin.siren.API.BeastEvents` - 18 events covering taming, hatching, summoning, a companion's own cultivation, and since 0.7.2 its arts, evolutions and mounts. Player-facing docs: [Spirit Beasts](/beasts/).
...
| `PreBeastArtEvent` | Yes | A companion is about to perform an art. Cancel to stop it — the cooldown is only stamped once the effect has actually run, so a vetoed art costs the beast nothing and it tries again on its next opening. | `owner()`, `player()`, `beast()`, `art()`, `commanded()` (false when the beast chose it itself). |
| `BeastArtEvent` | No | A companion performed one of its arts. | `owner()`, `player()`, `beast()`, `art()`, `commanded()`. |
| `PreBeastEvolveEvent` | Yes | A companion is about to be put through the evolution ritual. The Qi has **not** been taken at this point, so a veto costs the cultivator nothing. | `owner()`, `player()`, `beast()`, `from()`, `to()`, `successChance()` / setter. |
| `BeastEvolveEvent` | No | The ritual resolved. `succeeded()` says which way, and `to()` is null on failure. | `owner()`, `player()`, `beast()`, `from()`, `to()`, `succeeded()`. |
| `PreBeastMountEvent` | Yes | A companion is about to be summoned in its rideable body. Cancel to refuse the mount. | `owner()`, `player()`, `beast()`, `species()`. |
| `BeastMountEvent` | No | The companion was summoned in its rideable body. | `owner()`, `player()`, `beast()`, `species()`. |

Config keys these override: `PreBeastTameAttemptEvent.setChance` corresponds to `Tame-Chance-Bonus-Per-Realm`, `Tame-Chance-Element-Match-Bonus` and `Tame-Chance-Max` (with `Tame-Consume-Talisman-On-Failure` deciding what a miss costs); `PreBeastXpGainEvent.setAmount` to `Beast-Xp-Per-Kill`, `Beast-Xp-Own-Kill-Multiplier` and `Beast-Xp-Per-Qi-Item-Point`. Whole-system gate: `Beasts-Enabled`, with `Beast-Arts-Enabled`, `Beast-Evolution-Enabled` and `Beast-Mounts-Enabled` gating the three 0.7.2 systems independently.

<Divider />

## Sects

`plugin.siren.API.SectEvents` - 29 events covering founding, membership, ranks, halls, hall inscriptions and banners. Player-facing docs: [Sects](/sects/), and the `/sect` suite on the [Commands](/commands/) page.
...
| `PreSectBannerChangeEvent` | Yes | A sect's banner is about to change. Cancel to keep the old one flying. The id is **not** re-validated after a listener rewrites it - an unregistered id is not an error, it simply resolves to the vein-tier default light. | `manager()`, `sect()`, `oldBannerId()`, `bannerId()` / `setBannerId(String)`. |
| `SectBannerChangeEvent` | No | The banner changed. The hall's standing light swaps to it on its next pulse. | `manager()`, `sect()`, `oldBannerId()`, `newBannerId()`. |

Config keys these override: `PreSectInviteEvent.setExpiryMillis` corresponds to `Sect-Invite-Expiry-Seconds`; hall claims are gated by `Sect-Hall-Min-Vein-Tier`; inscriptions by `Sect-Inscription-Enabled`; the whole subsystem by `Sects-Enabled` and `Sect-Max-Members`. The banner ids the banner events carry are whatever is in the banner registry - the six built-ins plus anything an addon added through [`registerSectBanner`](/api-registries/).

<Divider />

## Wars

`plugin.siren.API.WarEvents` - 5 events covering declaring a siege on a rival's hall and how it resolves. Player-facing docs: [Sect Wars](/wars/).
...
`plugin.siren.API.DuelEvents` - 8 events covering challenging, accepting, and how a Qi-wager duel resolves. Player-facing docs: [Duels](/duels/), and `/cultivation duel` on the [Commands](/commands/) page.
...
`plugin.siren.API.FormationEvents` - 6 events covering laying and dispersing chunk-anchored spirit arrays, and the Trapping array wounding an intruder. Player-facing docs: [Formations](/formations/).
...
`plugin.siren.API.DwellingEvents` - 12 events covering claiming a Cave Abode (洞府), its Spirit Spring, upkeep, and closed-door seclusion. Player-facing docs: [Cave Abodes](/dwelling/).
...
`plugin.siren.API.BodyTemperingEvents` - 4 events covering the second progression ladder, the one climbed by taking the hit rather than by gathering Qi. Player-facing docs: [Body Tempering](/body-tempering/).

| Event | Cancellable | Fires | Exposes |
| --- | --- | --- | --- |
| `PreXpGainEvent` | Yes | A cultivator is about to earn tempering XP from damage that reached them. The amount is **editable**, not merely refusable — this is the supported way to retune the whole ladder for one player or one situation. | `ref()`, `player()`, `amount()` / `setAmount(float)`, `level()`. |
| `XpGainEvent` | No | The XP was banked. | `ref()`, `player()`, `amount()`, `level()`. |
| `PreLevelUpEvent` | Yes | A tempering level is about to be gained. Cancel to hold the cultivator where they are. | `ref()`, `player()`, `fromLevel()`, `toLevel()`. |
| `LevelUpEvent` | No | The level was gained. Fires once per level when a single blow crosses several. | `ref()`, `player()`, `fromLevel()`, `toLevel()`. |

Whole-system gate: `Body-Tempering-Enabled`. Note that XP is measured *after* armor, so a listener that reads `amount()` is seeing what actually got through, not what was swung.

## Cultivation profiles

`plugin.siren.API.ProfileEvents` - 7 events covering a player's saved cultivators: switching between them, creating one, erasing one, and a test profile expiring. Player-facing docs: [Profiles](/profiles/).

These carry a `Profile` payload (`CultivationProfilesComponent.Profile`) - useful members are `getName()`, `isTest()`, `getExpiresAtMillis()` and `remainingMillis(long)`. Note there is **no** pre-event for expiry: a test profile's time limit is not vetoable.

**These listeners may write.** Uniquely in the API, profile listeners run on the owning world thread *outside any ticking system*, so reading *and writing* components directly through the `Store` is safe here - the one exception to the usual "never write from a listener" rule. That is deliberate: a progression addon must be able to save and load its own state during a switch (see [`ProgressionProvider.supportsProfiles()`](/api-addons/)).

| Event | Cancellable | Fires | Exposes |
| --- | --- | --- | --- |
| `PreProfileSwitchEvent` | Yes | A player is about to change cultivators. Cancel to refuse the switch - the command reports it blocked. | `ref()`, `player()`, `from()` (null on a first-time backfill), `to()`. |
| `ProfileSwitchEvent` | No | The switch committed - the arriving profile's realm, Qi, race, dao, skill tree and techniques are already on the components. Load your own per-profile state here. | `ref()`, `player()`, `from()` (null on a first-time backfill), `to()`. |
| `PreProfileCreateEvent` | Yes | A fresh cultivator is about to be created. Cancel to refuse it - no slot is used. | `ref()`, `player()`. |
| `ProfileCreateEvent` | No | The profile exists and the player is on it. | `ref()`, `player()`, `profile()`. |
| `PreProfileDeleteEvent` | Yes | A save is about to be erased for good. Cancel to keep it. | `ref()`, `player()`, `profile()`. |
| `ProfileDeleteEvent` | No | The save is gone. Erase your own state for it here. | `ref()`, `player()`, `profile()`. |
| `ProfileExpireEvent` | No | A test profile's wall-clock limit ran out. `wasActive()` says whether the player was playing it - and so has just been put back on a real cultivator. | `ref()`, `player()`, `profile()`, `wasActive()`. |

If your addon keeps progression of its own, handle `PreProfileSwitchEvent` (save the profile being left) and `ProfileSwitchEvent` (load the arriving one), then return `true` from `ProgressionProvider.supportsProfiles()` - until you do, switching is refused outright while your provider is installed rather than half-performed. The caps are addon-raisable too: `registerProfileCap` on the [API facade](/api-reference/) lifts the three-slot default (ceiling six, highest registration wins, and nothing is ever destroyed when a cap falls).

<Divider />

## See also

- [API Registries](/api-registries/) - adding your own races, techniques, Qi absorption items, titles, sect banners and palettes.
- [API Reference](/api-reference/) - reading a player's cultivation state directly.
- [Config](/config/) - every server-owner-facing key an event lets you override per-player.
- [Commands](/commands/) - the commands most of these events sit behind.
