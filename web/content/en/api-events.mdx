...
Cultivation exposes 156 events across twelve `*Events` classes in `plugin.siren.API`. Nearly every mechanic in the mod is exposed twice - once as a cancellable `Pre*` event fired before the change, and once as a plain post-event fired after it is committed.
