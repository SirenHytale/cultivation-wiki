...
Sitting down to [meditate](/qi-gathering/) gives you a second, different aura. Where the realm aura rises and spreads, this one **converges inward** — Qi being drawn out of the land toward you. It deepens through three stages the longer you sit:
...
Both live in the root `Config.json` — see the [Core config page](/config-core/). The colors themselves are **baked into the particle assets** rather than configurable — recoloring them means editing `Server/Particles/Cultivation/RealmAura/` (the mod's `tools/gen_realm_auras.py` regenerates the set).
