...
每个方法都有一个「不予置评」的默认实现，故你只需实现自己在意的那几条。通道共十六条，涵盖灵气吸纳与打坐、所施与所受的伤害、空手伤害、伤害类型免疫、[蓄力](/zh/techniques/#gathered-arts)时长、大道元素之锁、大道亲和、阴阳偏移、阴阳容差、仪轨难度、于岩浆中打坐，以及[灵光](/zh/auras/)大小 —— 另有两条并不改动数值，而是向总览页添入条目。
...
[Cultivation: Sacred Bodies](/sacred-bodies/) 全然建于此注册表之上，别无其他，值得当作可仿的范式来读：它以**一次组件读取**回答全部通道，并向总览页添入自己的具名条目，使体质显现为可读的文字，而非玩家数值上一处无从解释的变动。
