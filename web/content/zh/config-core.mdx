...
这是唯一一份位于 `mods/Siren_Cultivation/` 根目录的文件。它承载着推导出每个境界与子阶段所需灵气的那条曲线，以及修士每攀升一级所得的固定属性加成。若你只打算改动一份文件来让服务器更难或更宽松，那就是它。曲线产出的结果见[境界与阶段](/zh/realms/)，灵气本身从何而来见[聚灵采气](/zh/qi-gathering/)。

## Config

核心灵气曲线、每级属性加成，以及[气息](/zh/auras/)与[屏显通告](/zh/notices/)的全服开关，位于路径 `mods/Siren_Cultivation/Config.json`。
...
| "Aura-Enabled" | true | [气息](/zh/auras/)的全服总开关 —— 境界气息与打坐气息皆归其管。只想藏起自己的气息、或不愿看见他人气息的玩家，在设置中另有各自独立的三个开关可用。 |
...
| "Notice-Titles-Enabled" | true | 境界突破、子阶段进阶、跌落境界与宗门变动时所现的[全屏标题](/zh/notices/)。聊天栏仍会道出这些标题所现的一切，故关闭它们无损分毫信息。 |
| "Notice-Notifications-Enabled" | true | 较次要时刻所现的[角落通知](/zh/notices/) —— 如解锁一个天赋节点、[本命法宝](/zh/lifebound/)升级、一次洗点，或一场仪式开始。 |
...
另有两套系统会削减玩家实际付出的量：第 8 层的「灵气消耗减免」天赋属性（其上限由[天赋树配置](/zh/config-cultivation/)中的 `Qi-Cost-Reduction-Cap-Percent` 约束），以及种族的 `Qi-Gain-Rate-Percent-Bonus`（见[种族配置](/zh/config-race/)）—— 后者提高的是收入，而非降低成本。
