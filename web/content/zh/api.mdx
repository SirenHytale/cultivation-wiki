...
- **读取**玩家的修炼状态 —— 境界、阶段、全局等级、存蓄灵气、种族、打坐状态、天赋点、已解锁节点。见[接口参考](/zh/api-reference/)。
- **注册**与内置内容行为完全一致的新内容 —— 一个种族、一门功法、一项灵气吸收物品修正。见[注册表](/zh/api-registries/)。
- **监听**（并重新调参，或否决）遍及模组各子系统的160 个事件。见[事件](/zh/api-events/)。
- **替换**模组的一整个层次 —— 进阶阶梯本身、它的全套用词，或两者兼有 —— 而其余每个子系统仍照常在其之上运行。此为 0.5.0 新增；见[编写扩展](/zh/api-addons/)。
...
| 页面 | 涵盖 |
| --- | --- |
| [事件](/zh/api-events/) | 十二个 `*Events` 类中的每一个事件、它暴露什么，以及取消它意味着什么。 |
| [注册表](/zh/api-registries/) | `registerRace`、`registerTechnique`、`registerQiAbsorptionItemModifier`、`registerTitle`、`registerSectBanner`、`registerPalette` 以及可由扩展提升的上限。 |
| [接口参考](/zh/api-reference/) | 组件类型的 getter，以及 `CultivationAPI` 门面上的各项状态读取。 |
| [编写扩展](/zh/api-addons/) | `ProgressionProvider`（包括 `supportsProfiles`）、`CultivationTheme` 与 `AdminConfigSection` —— 替换阶梯、替换用词，以及添加你自己的管理设置。 |

这些钩子所依托的各系统，其面向玩家的文档位于本站主体：[境界与阶段](/zh/realms/)、[聚灵采气](/zh/qi-gathering/)、[大道](/zh/dao/)、[功法](/zh/techniques/)、[宗门](/zh/sects/)、[宗门攻伐](/zh/wars/)、[阵法](/zh/formations/)、[洞府](/zh/dwelling/)、[斗法](/zh/duels/)与[灵兽](/zh/beasts/)。事件允许你重新调整的每一个数值，在[配置](/zh/config/)页都有一份面向服主的对应项，其中大多数也在[指令](/zh/commands/)页有所映照。
